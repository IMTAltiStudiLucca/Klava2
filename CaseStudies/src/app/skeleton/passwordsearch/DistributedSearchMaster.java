...
public class DistributedSearchMaster<T extends ITupleSpace> {
     ...
     */
  	public void passwordSearchMaster() throws NoSuchAlgorithmException, InterruptedException, FileNotFoundException, IOException {
        ...
        TupleOperations.writeTuple(masterTS, masterTS, masterTS.formTuple("SearchTuple_tttt", new Object[]{"search", "profiling", "master_key", DProfiler.testKey}, DistributedSearchMaster.searchTupleTemplate), true, false);
        ...
        for(int i =0; i< numberOfWorkers; i++) {
        	TupleOperations.writeTuple(masterTS, masterTS, masterTS.formTuple("SearchTuple_tttt", new Object[]{"search_task", "hashed_value", "", completeStatus}, searchTupleTemplate), true, false);
        }
...
//        		"write::local", "takeE::local", "takeE::remote", "read::l-r", "nodeVisited", "Master::TotalRuntime", "match"});
	...
	private void waitForAllWorkers(T masterTS) throws InterruptedException {
        ...
        while (true) {
        	Thread.sleep(10);
        	TupleOperations.takeTuple(masterTS, masterTS, masterTS.formTuple("SearchTuple_tttt", new Object[]{"search", "sync", "worker", "worker_ready"}, searchTupleTemplate), true, false);
        	workerCounter++;
	...
	private void waitForDataLoad(T masterTS) throws InterruptedException {
        ...
        while (true) {
        	Thread.sleep(10);
        	TupleOperations.takeTuple(masterTS, masterTS, masterTS.formTuple("SearchTuple_tttt", new Object[]{"search", "sync", "worker", "data_loaded"}, searchTupleTemplate), true, false);
        	workerCounter++;
        	if(workerCounter == numberOfWorkers)
        		break;     
        }
	...
	private void taskCreation(T masterTS, int numberOfTasks) throws NoSuchAlgorithmException {
        ...
        for(int i = 0; i < tasks.size(); i++) {    	
        	TupleOperations.writeTuple(masterTS, masterTS, 
        			masterTS.formTuple("SearchTuple_tttt", new Object[]{"search_task", "hashed_value", tasks.get(i), "not_processed"}, searchTupleTemplate), true, false);
        }
	...
	private ArrayList<Object[]> getAccomplishedTasks(T masterTS, int numberOfTasks) {
        ...
        while (foundTuples.size() != numberOfTasks) {        	
        	Object foundTupleTemplate = masterTS.formTuple("SearchTuple_tttt", new Object[]{"results", "found_value", null, null}, searchTupleTemplate);

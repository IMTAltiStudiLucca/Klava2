...
/*
    ...
    public Vector<Tuple> getBoundaryValues() {
        ...
        Scope scope = new Scope(Scope.EMPTY_SCOPE);

        Vector<Tuple> vals = new Vector<Tuple>();

        if(SHARE_LEFT) { 
            for(int j = 0; j < length-1; j++) {
                vals.addElement(new Tuple(scope, "u", 1, j,
        ...
        if(SHARE_RIGHT) {
            for(int j = 0; j < length-1; j++) {
                vals.addElement(new Tuple(scope, "eta", width, j,
                                        new Double(eta[width-1][j][keta_new]),
                                        new Integer(keta), new Integer(id)));
            }

            r = new Tuple(scope, "eta_inter", id, keta, vals);
        }
    ...
    public Vector<Vector> getBoundaryValuesGen() {
        Vector l = null;
        Vector r = null;
       
        Vector<Vector> vals = new Vector<Vector>();

        if(SHARE_LEFT) { 
            for(int j = 0; j < length-1; j++) {
            	Vector tupleFields = new Vector<>();
            	tupleFields.addElement("u");
               	tupleFields.addElement(1);
            	tupleFields.addElement(j);
            	tupleFields.addElement(new Double(u[1][j][kuv_new]));
            	tupleFields.addElement(new Integer(kuv));
            	tupleFields.addElement(new Integer(id));
            	
            	vals.add(tupleFields);
//                vals.add(new Tuple("u", 1, j,
//                                        new Double(u[1][j][kuv_new]),
//                                        new Integer(kuv), new Integer(id)));
            }

            l = new Vector();
            l.addElement("u_inter");
            l.addElement(id);
            l.addElement(kuv);
            l.addElement(vals);
        }
        ...
        if(SHARE_RIGHT) {
            for(int j = 0; j < length-1; j++) {
            	Vector tupleFields = new Vector<>();
            	tupleFields.addElement("eta");
               	tupleFields.addElement(width);
            	tupleFields.addElement(j);
            	tupleFields.addElement(new Double(eta[width-1][j][keta_new]));
            	tupleFields.addElement(new Integer(keta));
            	tupleFields.addElement(new Integer(id));
            	
            	vals.add(tupleFields);
//                vals.add(new Tuple("eta", width, j,
//                                        new Double(eta[width-1][j][keta_new]),
//                                        new Integer(keta), new Integer(id)));
            }

            r = new Vector();
            r.addElement("eta_inter");
            r.addElement(id);
            r.addElement(keta);
            r.addElement(vals);
            
          //  r = new Vector("eta_inter", id, keta, vals);
        }
    ...
    public Vector<Vector> getBoundaryTemplatesGen() {
        Vector<Vector> templates = new Vector<Vector>();
    ...
    public void updateBoundariesGen(Vector<Vector> vals) {
        for(Vector val : vals) {
            int j = (Integer) val.get(2);
            String array = (String) val.get(0);

            if(array.equals("u"))
                u[width][j][kuv_new] = (Double) val.get(3);
            else if(array.equals("eta"))
                eta[0][j][keta_new] = (Double) val.get(3);
        }
    }

    /*
    // Return Template objects suitable for retrieving boundary data points 
    ...
    public void init() {
    	System.out.println("panel init " + width + " x " + length);

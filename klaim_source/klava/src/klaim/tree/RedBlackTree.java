...
public class RedBlackTree<T> {
    ...
    private void insert(RedBlackNode<T> z) {
            ...
            if (isNil(y))
                root = z;
            else if (comparator.compare(z.key, y.key) < 0)       // (z.key.compareTo(y.key) < 0)
                y.left = z;
            else
                y.right = z;
    ...
    public boolean remove(RedBlackNode<T> v){
        ...
        while (!isNil(current)){
            ...
            if (y.key != current.key) {
                ...
                // current.key then decrement current.numRight
                 if (comparator.compare(y.key, current.key) > 0 )// if (y.key.compareTo(current.key) > 0)
                    current.numRight--;
    ...
    public RedBlackNode<T> search(T key){
        ...
        while (!isNil(current)){
            ...
            else  if (comparator.compare(current.key, key) < 0)  // if (current.key.compareTo(key) < 0)
    ...
    public int findNumGreater(RedBlackNode<T> node, T key){
        ...
        else if (comparator.compare(key, node.key) < 0)   //if (key.compareTo(node.key) < 0)
    ...
    private void getGreaterThan(RedBlackNode<T> node, T key,
        ...
        if (isNil(node)) {
            return;
        } else if (comparator.compare(node.key, key) > 0) {   // if (node.key.compareTo(key) > 0) {
